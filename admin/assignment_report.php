...
// Assignment filters
$assignment_colleges = $db->query("SELECT DISTINCT college FROM faculties ORDER BY college")->fetchAll();
...
if ($assignment_college) {
    $stmt = $db->prepare("SELECT DISTINCT DATE(a.created_at) FROM assignments a JOIN sections s ON a.section_id = s.id JOIN faculties faculty ON s.faculty_id = faculty.id WHERE faculty.college = ? ORDER BY DATE(a.created_at) DESC");
...
if (!$assignment_college && !$assignment_date) {
    ...
    $stmt = $db->prepare("SELECT faculty.college, SUM(a.status) as submitted, COUNT(a.id) as total, AVG(a.score) as avg_score FROM assignments a JOIN sections s ON a.section_id = s.id JOIN faculties faculty ON s.faculty_id = faculty.id WHERE DATE(a.created_at) = ? GROUP BY faculty.college");
    $stmt->execute([$today]);
    ...
    $stmt = $db->prepare("SELECT s.id as section_id, s.name as section_name, AVG(a.score) as avg_score, COUNT(a.id) as total FROM sections s JOIN faculties faculty ON s.faculty_id = faculty.id LEFT JOIN assignments a ON a.section_id = s.id AND DATE(a.created_at) = ? WHERE faculty.college = ? GROUP BY s.id, s.name ORDER BY s.name");
            ...
            <div class="col-md-3 col-lg-2 px-0 d-none d-md-block">
                <div class="sidebar">
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link <?php echo $current_page == 'manage_faculties.php' ? 'active' : ''; ?>"
                            href="manage_faculties.php">
