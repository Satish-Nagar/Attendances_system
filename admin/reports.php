...
if ($assignment_college) {
    $stmt = $db->prepare("SELECT DISTINCT DATE(a.due_date) FROM assignments a JOIN sections s ON a.section_id = s.id JOIN faculties faculty ON s.faculty_id = faculty.id WHERE faculty.college = ? ORDER BY DATE(a.due_date) DESC");
...
if ($assignment_college && $assignment_date) {
    $stmt = $db->prepare("SELECT s.name AS section_name, AVG(sc.score) AS avg_score, SUM(sc.status) AS submitted, COUNT(sc.id) AS total FROM assignment_scores sc JOIN assignments a ON sc.assignment_id = a.id JOIN sections s ON sc.section_id = s.id JOIN faculties faculty ON s.faculty_id = faculty.id WHERE faculty.college = ? AND DATE(a.due_date) = ? GROUP BY s.name");
