...
class PasswordReset {
    ...
    /**
     * Check if email exists in either admin or Faculty table
     */
    public function checkEmailExists($email) {
        ...
        $stmt = $this->pdo->prepare("SELECT id, 'faculty' as user_type FROM faculties WHERE email = ?");
        $stmt->execute([$email]);
        $faculty = $stmt->fetch();
    ...
    public function updatePassword($token, $new_password) {
        ...
        if ($token_data['user_type'] === 'admin') {
            ...
            $stmt = $this->pdo->prepare("UPDATE faculties SET password = ? WHERE email = ?");
        }
    ...
    public function sendResetEmail($email) {
        ...
        $reset_link = getAppUrl('reset_password.php') . '?token=' . urlencode($token);
