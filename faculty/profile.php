...
// Fetch Faculty profile picture for header
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $stmt = $db->prepare("UPDATE faculties SET name = ?, contact = ?, profile_picture = ? WHERE id = ?");
        if ($stmt->execute([$name, $contact, $profile_picture, $faculty_id])) {
...
<html lang="en">
  ...
  .faculty-header-bar {
        ...
        .faculty-header-bar .header-title {
            font-weight: bold;
            font-size: 1.4rem;
            display: flex;
            align-items: center;
        }
        .faculty-header-bar img {
            height: 40px;
            width: auto;
            margin-right: 10px;
            border-radius: 25%;
        ...
        /* Profile image styling for PNG transparency in profile page */
        .img-thumbnail[src*=".png"] {
            background-color: white;
        }
        /* Sidebar */
        .sidebar {
  ...
  <body>
    <!-- Header -->
    <div class="faculty-header-bar d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center ms-auto me-2 d-lg-none">
            <div class="nav-item dropdown d-flex align-items-center profile-dropdown">
                ...
                <a class="nav-link dropdown-toggle fw-bold text-white ms-3" href="#" role="button" data-bs-toggle="dropdown">
                    <span class="d-none d-lg-inline"><?php echo htmlspecialchars($_SESSION['faculty_name'] ?? 'Faculty'); ?></span>
                </a>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="alert alert-info">
                ...
                Profile Picture: <?php echo $faculty['profile_picture'] ? 'Yes' : 'No'; ?><br>
                Image Path: <?php echo $profile_img_src; ?><br>
                File Exists: <?php echo file_exists('../uploads/' . $faculty['profile_picture']) ? 'Yes' : 'No'; ?>
            </div>
            <?php endif; ?>
            <div class="card mb-4">
                ...
                <form method="POST" enctype="multipart/form-data">
                  <div class="row mb-3">
                    <div class="col-md-3 text-center">
                      <?php 
                      $profile_img_src = $faculty['profile_picture'] ? '../uploads/' . htmlspecialchars($faculty['profile_picture']) : 'https://via.placeholder.com/120x120.png?text=Profile';
                      ?>
                      <img
                        src="<?php echo $profile_img_src; ?>"
                        class="img-thumbnail mb-2"
                        style="max-width: 120px; height: 120px; object-fit: cover;"
                        alt="Profile Picture"
                        onerror="this.src='https://via.placeholder.com/120x120.png?text=Profile'"
                      />
                      <input
                        type="file"
                    ...
                    <div class="col-md-9">
                      ...
                      <div class="mb-3">
                        ...
                        <input
                          ...
                          name="contact"
                          class="form-control"
                          value="<?php echo htmlspecialchars($faculty['contact']); ?>"
                          required
                        />
                      </div>
                      <div class="mb-3">
                        ...
                        <input
                          type="text"
                          class="form-control"
                          value="<?php echo htmlspecialchars($faculty['college']); ?>"
                          disabled
                        />
                      </div>
