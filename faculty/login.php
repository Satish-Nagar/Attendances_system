...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $db = getDB();
        $stmt = $db->prepare("SELECT id, name, email, password, is_active FROM faculties WHERE email = ?");
        $stmt->execute([$email]);
        $faculty = $stmt->fetch();
        ...
        if ($faculty && verifyPassword($password, $faculty['password'])) {
            if (!$faculty['is_active']) {
                ...
                $_SESSION['om_email'] = $faculty['email'];
                
                setFlashMessage('success', 'Welcome back, ' . $faculty['name'] . '!');
                redirect('dashboard.php');
            }
        } else {
...
<html lang="en">
  <head>
    ...
    <style>
      body {
        background: linear-gradient(135deg, #0B1C3A 0%, #1a3a6e 100%);
        min-height: 100vh;
        display: flex;
